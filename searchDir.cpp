...
* Return: Nothing.
*/
void searchDir::sensitive(std::string command, wordHash* gerpHash, std::ostream& output){
    ...
    if(gerpHash->mainHash.inHash(word)){
        if(gerpHash->mainHash.get(word).inHash(command)){
            for(size_t i = 0; i<gerpHash->mainHash.get(word).get(command).size(); i++){
            ...
            output << command << " Not Found. Try with @insensitive or @i." << endl;
        }
    }else{
        output << command << " Not Found. Try with @insensitive or @i." << endl;
    }
...
* Return: Nothing.
*/
void searchDir::insensitive(std::string command, wordHash* gerpHash, std::ostream& output){
